...
public:
  explicit StreamParser(DataSource &source, std::FILE *out)
    : XmlParser(source), iOut(out) { /* nothing */ }
  bool parse();
...
{
  fprintf(stderr,
	  "Usage: ipeextract ( <input.pdf> | <input.eps> ) [<output.xml>]\n"
...
int main(int argc, char *argv[])
{
  ...
  if (format == EXml) {
    ...
    std::rewind(fd);
    std::FILE *out = std::fopen(dst.z(), "wb");
    if (!out) {
