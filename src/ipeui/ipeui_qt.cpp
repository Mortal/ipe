...
void PDialog::setMapped(lua_State *L, int idx)
{
  ...
  switch (m.type) {
  case ELabel:
    (qobject_cast<QLabel *>(w))->setText(QString::fromUtf8(m.text.z()));
    break;
  case ECheckBox:
    (qobject_cast<QCheckBox *>(w))->setChecked(m.value);
    break;
  case ETextEdit:
    (qobject_cast<QTextEdit *>(w))->setText(QString::fromUtf8(m.text.z()));
    break;
  case EInput:
    (qobject_cast<QLineEdit *>(w))->setText(QString::fromUtf8(m.text.z()));
    break;
  case EList:
    {
	...
	for (int k = 0; k < int(m.items.size()); ++k)
	  l->addItem(QString::fromUtf8(m.items[k].z()));
      }
  ...
  case ECombo:
	...
	b->clear();
	for (int k = 0; k < int(m.items.size()); ++k)
	  b->addItem(QString::fromUtf8(m.items[k].z()));
      }
...
bool PDialog::buildAndRun(int w, int h)
{
  for (int i = 0; i < int(iElements.size()); ++i) {
    ...
    if (m.row < 0) {
      QPushButton *b = new QPushButton(QString::fromUtf8(m.text.z()), this);
	...
	connect(b, SIGNAL(clicked()), SLOT(notifyLua()));
      ...
      switch (m.type) {
      case ELabel:
	w = new QLabel(QString::fromUtf8(m.text.z()), this);
	break;
      case EButton:
	{
	  QPushButton *b = new QPushButton(QString::fromUtf8(m.text.z()), this);
      ...
      case ETextEdit:
	{
	  ...
	  QString text = QString::fromUtf8(m.text.z());
	  if (m.flags & ELogFile) {
	    markupLog(t, text);
	  } else
	    t->setPlainText(text);
...
void PDialog::retrieveValues()
{
  for (int i = 0; i < int(iElements.size()); ++i) {
    ...
    case EInput:
      m.text =
	String((qobject_cast<QLineEdit *>(w))->text().toUtf8());
      break;
    case ETextEdit:
      m.text = String((qobject_cast<QTextEdit *>(w))->toPlainText().toUtf8());
      break;
    case EList:
      {
