...
static int ipeui_fileDialog(lua_State *L)
{
  ...
  // const char *filter = luaL_checkstring(L, 4); // not used
  const char *dir = 0;
  if (!lua_isnoneornil(L, 5))
    dir = luaL_checkstring(L, 5);
  // GTK dialog uses no filters:
...
{
  luaL_error(L, "'closeAllWindows' is not yet implemented.");
  return 0;
}
...
static int ipeui_local8bit(lua_State *L)
{
  ...
  gchar *r = g_locale_from_utf8(s, -1, NULL, NULL, NULL);
  if (r) {
    lua_pushstring(L, r);
    g_free(r);
    return 1;
