...
int ipelua::ipelet_constructor(lua_State *L)
{
...
#ifdef WIN32
  HMODULE hMod = LoadLibraryA(fname.z());
  if (hMod) {
...
#else
  void *handle = dlopen(fname.z(), RTLD_NOW);
  if (handle) {
  ...
  if (pIpelet) {
    ...
    *p = ipelet;
    ...
    lua_pushnil(L);
#ifdef WIN32
    lua_pushstring(L, "Error loading Ipelet DLL");
#else
    lua_pushfstring(L, "Error loading Ipelet: %s", dlerror());
#endif
    return 2;
  }
}
