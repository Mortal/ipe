...
Object::Object(const XmlAttributes &attr)
{
  ...
  if (attr.has("transformations", str) && !str.isEmpty()) {
    if (str == "rigid")
...
/*! Returns true if an attribute was actually changed.  The arguments
  \a stroke and \a fill are only used then \a prop is \e
  EPropPathMode, and stroking or filling is added to an object that
