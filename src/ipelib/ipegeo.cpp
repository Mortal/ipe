...
void Bezier::closedSpline(int n,  const Vector *v, std::vector<Bezier> &result)
{
  for (int i = 0; i < n; ++i) {
    ...
    Vector p2 = v[(i + 2) % n];
    Vector p3 = v[(i + 3) % n];
    Vector q1 = thirdpoint(p1, p2);
    Vector q2 = thirdpoint(p2, p1);
    Vector q0 = midpoint(thirdpoint(p1, p0), q1);
    Vector q3 = midpoint(thirdpoint(p2, p3), q2);
    result.push_back(Bezier(q0, q1, q2, q3));
  }
}
...
void intersectBeziers(std::vector<Vector> &intersections,
                      const Bezier &a, const Bezier &b)
{
  ...
  if (a.straight(PRECISION) && b.straight(PRECISION)) {
    Segment aLine = Segment(a.iV[0], a.iV[3]);
    Segment bLine = Segment(b.iV[0], b.iV[3]);

    Vector p;
    if (aLine.intersects(bLine, p)) {
      // We found an intersection point.  If this point is too close
      // to one of the outer control points, it is ignored.
...
void Bezier::intersect(const Line &l, std::vector<Vector> &result) const
{
  ...
  if (straight(PRECISION)) {
    ...
    if (Segment(iV[0], iV[3]).intersects(l, p)) {
      // We found an intersection point.  If this point is too close
      // to one of the outer control points, it is ignored.
      if ((p-iV[0]).len() > MIN_DIST && (p-iV[3]).len() > MIN_DIST)
        result.push_back(p);
    }
  } else {
    Bezier leftA, rightA;
